...
#define ARGASSERT(args, num, err) \
  if (args->count != num) { lval_del(args); return lval_err(err); }
...
/* A function that calls the correct builtin function
based on what symbol it encounters in evaluation */
lval* builtin(lval* a, char* func) {
  ...
  if (strstr("add+sub-div/mul*%minmax", func)) { return builtin_op(a, func); }
  lval_del(a);
...
/* A function to evaluate S-expressions */
...
/* A function to evaluate an Lval */
lval* lval_eval(lval* v) {
